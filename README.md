...
1. Clonez le dépôt :
```bash
git clone https://github.com/votre-username/FabBoard.git
cd FabBoard
```
...
# FabBoard - Gestion de Maintenance pour FabLabs
...
Première appli dont je n'ai quasiment pas vu le code ! -> <https://maintenance.resfabrica.fr/>

Je lance une expérimentation, c'est du code généré par IA. Je veux voir si c'est ok pour la cible (vous FabManager), si c'est pas trop bugué, si ça réponds au besoin. J'ai besoin de vos retours ! Testez et essayez, faites moi des retours, je pourrais améliorer l'appli ! Qu'est ce que c'est ? Je vous laisse l'IA en parler !

(Je le mettrais en open source si vous êtes intéressé, un petit pouce pour soutenir le projet)
...
Rendez-vous sur <https://maintenance.resfabrica.fr/accounts/login/?next=/>

Créez un compte avec votre email

Ajoutez votre premier FabLab

Commencez à gérer vos machines !
